...
(function () {
  ...
  /* One page carries one builder today, but the preview bundle puts several in
     a single document, so mounting is per-element rather than per-page. Each
     mount keeps its own state and reads the same saved list. */
  function mountQuote(mount) {
  ...
  document.querySelectorAll(".quote-builder").forEach(mountQuote);
})();
