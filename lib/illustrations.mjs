 ...
 * Schematic backdrops.
 *
 * A faint engineering-drawing layer — setting-out grid, dimension lines, a unit
 * in outline, a pipe run — sitting behind the hero and the closing call to
 * action. It signals "measured and planned" rather than "sales page", which is
 * the impression a customer spending several thousand pounds is looking for.
 *
 * Decorative only: aria-hidden, no pointer events, and kept faint enough that
 * text contrast over it is unaffected.
 */

const dimension = (x1, x2, y, label) => `
  <path d="M${x1} ${y}h${x2 - x1}" />
  <path d="M${x1} ${y - 6}v12M${x2} ${y - 6}v12" />
  <path d="M${x1 + 7} ${y - 4}l-7 4 7 4M${x2 - 7} ${y - 4}l7 4-7 4" fill="none"/>
  ${label ? `<text x="${(x1 + x2) / 2}" y="${y - 10}" text-anchor="middle" class="bd-text">${label}</text>` : ""}`;

export const schematicBackdrop = () => `
<svg class="backdrop" viewBox="0 0 1200 620" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
  <defs>
    <pattern id="bd-fine" width="24" height="24" patternUnits="userSpaceOnUse">
      <path d="M24 0H0v24" fill="none" stroke="currentColor" stroke-width=".7" opacity=".45"/>
    </pattern>
    <pattern id="bd-coarse" width="120" height="120" patternUnits="userSpaceOnUse">
      <path d="M120 0H0v120" fill="none" stroke="currentColor" stroke-width="1.1" opacity=".55"/>
    </pattern>
  </defs>

  <rect width="1200" height="620" fill="url(#bd-fine)"/>
  <rect width="1200" height="620" fill="url(#bd-coarse)"/>

  <g fill="none" stroke="currentColor" stroke-width="1.6" opacity=".5">
    <!-- room section, right of the hero copy -->
    <path d="M760 96h372M760 96v420M760 516h372"/>
    <path d="M784 120h300" stroke-dasharray="6 8" opacity=".6"/>

    <!-- indoor unit in outline, with its swing -->
    <rect x="852" y="150" width="168" height="40" rx="8"/>
    <path d="M868 176h136"/>
    <path d="M876 206c-8 22-26 38-50 46M936 210c-4 26-18 44-40 56M996 206c8 22 26 38 50 46"/>

    <!-- pipe run down to the condenser -->
    <path d="M1020 170h64c14 0 14 10 14 22v210" stroke-dasharray="10 7"/>
    <rect x="1046" y="402" width="112" height="76" rx="6"/>
    <circle cx="1102" cy="440" r="26"/>
    <circle cx="1102" cy="440" r="9"/>

    <!-- setting-out marks -->
    <path d="M760 300h-56M704 288v24"/>
    <circle cx="760" cy="96" r="4"/>
    <circle cx="760" cy="516" r="4"/>
  </g>

  <g class="bd-dim" fill="none" stroke="currentColor" stroke-width="1.2" opacity=".55">
    ${dimension(852, 1020, 224, "2.4 m")}
    ${dimension(760, 1132, 556, "")}
  </g>
</svg>`;

/* A lighter version for the closing band, which already carries a gradient. */
export const bandSchematic = () => `
<svg class="backdrop backdrop--band" viewBox="0 0 1200 320" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
  <defs>
    <pattern id="bd-band" width="40" height="40" patternUnits="userSpaceOnUse">
      <path d="M40 0H0v40" fill="none" stroke="currentColor" stroke-width=".8" opacity=".5"/>
    </pattern>
  </defs>
  <rect width="1200" height="320" fill="url(#bd-band)"/>
  <g fill="none" stroke="currentColor" stroke-width="1.6" opacity=".55">
    <rect x="72" y="70" width="150" height="36" rx="7"/>
    <path d="M88 92h118"/>
    <path d="M96 122c-6 18-20 30-40 36M156 126c-3 20-14 34-30 44"/>
    <path d="M222 88h58c12 0 12 8 12 18v104" stroke-dasharray="9 6"/>
    <rect x="978" y="86" width="126" height="86" rx="6"/>
    <circle cx="1041" cy="129" r="30"/>
    <circle cx="1041" cy="129" r="10"/>
    <path d="M940 129h-70M870 116v26"/>
  </g>
</svg>`;
